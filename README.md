...
- Non-type параметры: ``` template<class T, size_t N> class array; ```
...
## Макрос
Макрос - это механизм преобразования кода препроцессором в текст, выполняемые ДО передачи этого тескта компилятору. С текстом программы работает препроцессор, для которого программа - набор символов (букв, цифр, знаков для операторов, пробелов, и т.д.).
